...
While the actual instellation threshold for a runaway climate depends on planetary albedo, surface gravity, and clouds~\citep{Pierrehumbert2022}, this value was found to be a typical limit for the flux a planet can emit in a runaway greenhouse situation~\citep{Goldblatt2013,Leconte2013}.
...
Further, a planet in a runaway greenhouse state will eventually lose its steam atmosphere and move to category~A on a timescale of $10^8\SI{}{\year}$ for a planet with one Earth ocean of water~\citep[][]{Watson1981}.
...
Our simulation setup is such that all planets receiving a net instellation $S < S_\mathrm{thresh}$ follow relation~A, and a fraction $f_\mathrm{rgh}$ of the planets with $S > S_\mathrm{thresh}$ follow relation~C.
...
(How close is Earth to runaway? Could anthropogenic climate change push us over the edge? (-> \citet{Goldblatt2012,Popp2016}; what is the max. expected change in Earth's albedo due to deglaciation?)}

\subsection{Statistical imprint of exoplanet climates}
\todo[inline]{what is the fraction of Kepler planets w/ run-
away GH?}
\begin{note}
...
\begin{itemize}
    \item Occurrence rate of planets forming steam atmospheres: The runaway greenhouse climate relies on the production of large amounts of water vapor that then acts as a greenhouse gas. This process works only on planets with an atmosphere and of sufficiently volatile-rich composition\todo[inline]{how much?}. The fraction of planets fulfilling these requirements has an impact on the amplitude of the demographic imprint of the runaway greenhouse transition.
    \item Planetary evolution and duration of the steam atmosphere phase: An inflated steam atmosphere can only be sustained until the planet has lost its water. Depending on host star spectral type and planetary composition, planets spend from a few Myr to hundreds of Myr in runaway greenhouse climates~\citep[e.g.,][]{Luger2015}, and only planets observed during this phase will contribute to the signal.
...
As in the cases above, an M~dwarf survey profits from follow-up measurements of planetary masses with an order of magnitude increase in evidence.
    ...
    \todo[inline]{transit depth difference is larger in M dwarfs \citep{Turbet2019}}
...
%The authors thank Yann Fabel, Paul Molliere, Gijs Mulders, Matthew Murphy, and Terry-Ann Suer for insightful discussions.
