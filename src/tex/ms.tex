...
This is not the case in the model from \citet{Bergsten2022} that we adopted: Its occurrence rate density is smooth in the domain relevant for the runaway greenhouse transition; discontinuities only occur at smaller instellations ($< \SI{50}{\watt\per\meter\squared}$) and larger radii ($> 1.6\,R_\oplus$).
Due to the flat occurrence rate in the region of interest, we do not expect the model underlying our planet sample to affect the recovery of the transition.
