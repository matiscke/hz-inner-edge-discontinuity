...
\subsubsection{Underlying occurrence rate distributions}

Our adopted occurrence rate model \citep{Bergsten2022} changes forms at larger separations ($> \SI{50}{\watt\per\meter\squared}$) and radii ($> 1.6\,R_\oplus$) than what is relevant for the runaway greenhouse threshold, where the adopted occurrence rate density is relatively smooth.
Thus we argue that the proposed trend is not an artifact of the underlying occurrence distribution.
However, this may be different for alternative models that have sharp features in relevant regions of $R_P - S$ space. 
\todo[inline]{More discussion goes here?}
