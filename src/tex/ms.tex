...
From this catalog, \bioverse\ samples stars within an isotropic distance from the Solar System as required by the sample size.
...
\todo[inline]{GALEN: The most relevant assumption of using our model is that, at the relevant fluxes (I think), we predict that super-Earths are pretty common. I think the "drop" in our model where super-Earths become less common happens at later fluxes than what is relevant in this paper... Let's double check this, and add a note in the first paragraph about it?.}

\subsection{Orbit parameters and planet masses}
...
\section{Exoplanet Survey Simulations and Hypothesis Testing}
...
\subsection{Detection bias, sample selection, and sensitivity}\label{sec:sensitivity} % Turbet2019 lists precision for CHEOPS, PLATO?, TESS?
...
However, for a transit signal to be detected in a transit survey, it must reach a sufficient signal-to-noise ratio (S/N) that is sensitive on the achieved photometric precision~\citep[e.g.,][]{Burke2015,Hardegree-Ullman2019}.
...
For uncertainties in bulk density measurements, we propagate normally distributed errors of mass measurements assuming $\sigma_\mathrm{M_\mathrm{P}} = \SI{10}{\percent}$, as expected for \plato. \todo[inline]{20230307 after restructuring, check where the above sentence should go}
...
\subsection{The runaway greenhouse hypothesis}
...
\todo[inline]{Mention multiplanet systems.}
...
\todo[inline]{what about TESS?}

Taken together, \plato\ appears to be the most promising exoplanet mission for an empirical confirmation or falsification of the runaway greenhouse transition at this time.
...
This way, useful constraints can be obtained under more pessimistic conditions, e.g., a low predominant water content of planetary surfaces and atmospheres or a smaller available planet sample.
...
However, precise ground-based radial velocity measurements are expensive, and therefore such follow-up efforts may only be available for a subsample of a mission's targets.
...
''present bracketing cases; or to discuss why the discountinuity in the results would not be affected by changes that led to continuous offsets in the results. Or both.''\\
- not yet explored: how sensitive are we on the assumed planet population? $\exists$ danger of false positive detection on occurrence rate ``ridges'' like the Neptune desert? Currently we have a relatively smooth occurrence rate density in $R_P - S$ space based on the power-law fits in \citet{Bergsten2022}. We could, with relatively moderate effort, run a sanity check on an alternative sample (e.g., the more structured Bern model)}
